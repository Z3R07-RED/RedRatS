...
if [[ -f "$text_file" ]]; then
    ...
    touch "${text_file}"
    echo "- Escribe tu texto aquí ..." >> "${text_file}"
    echo "- Luego presione la tecla 'TAB' y luego 'ENTER'" >> "${text_file}"
    printf "\n" >> "${text_file}"
    printf "\n" >> "${text_file}"
    printf "\n" >> "${text_file}"
    printf "\n" >> "${text_file}"
...
if [[ "$selected_sound_option" == 1 ]]; then
    ...
    if [[ -f "$converter_tool/sounds/$selected_audios" ]]; then
        _rename_audio=$($DIALOG --stdout --backtitle "$program_name - $current_tool_name" \
                                --title "RENAME" \
                                --ok-label "Rename" \
                                --inputbox "Cambiar el nombre del archivo:" 10 51 "$selected_audios")
        ...
        case $? in
            0)
                if [[ -n "$_rename_audio" ]]; then
                    ...
                    $DIALOG --backtitle "$program_name - $current_tool_name" \
                            --title "INFORMATION" \
                            --msgbox "Le nombre del Archivo '$selected_audios' Ha cambiado ah: '$_rename_audio'" 9 60
                else
                    unexpected_error
                fi
                ;;
...
if [[ "$sound_directory" == 0 ]]; then
    $DIALOG --backtitle "$program_name - $current_tool_name" \
            --title "INFORMATION" \
            --msgbox "¡Aún no se ha creado ningún audio!" 8 40
    break
fi
...
selected_audios=$($DIALOG --scrollbar --backtitle "$program_name - $current_tool_name" \
                ...
                --radiolist "Presione la tecla \Z4\Zu\Zb'ESPACIO'\Zn y luego \Z4\Zu\Zb'ENTER'\Zn" \
...
do
    $DIALOG --backtitle "$program_name - $current_tool_name" \
        --title "CONVERTER" \
        --infobox "Conversión de texto a audio mp3 ..." 8 46; sleep 3.6
    ...
    $DIALOG --backtitle "$program_name - $current_tool_name" \
        --title "CONVERTER" \
        --infobox "Espere por favor..." 8 41; sleep 3
done

$DIALOG --backtitle "$program_name - $current_tool_name" \
    --title "CONVERTER" \
    --msgbox "El texto se ha convertido a audio correctamente." 8 50
